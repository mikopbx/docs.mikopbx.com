...
* Импортирует события `transcript.completed` из модуля локального распознавания речи.
* Сохраняет ссылки и ограниченные по сроку снимки готовых расшифровок.
* Создаёт задания `call_summary` для ещё не проанализированных звонков.
* Выдаёт задания только зарегистрированным обработчикам с отдельным ключом доступа.
* Управляет составом анализа, профилем модели, языком результата, временем ожидания и повторами.
* Поддерживает сводку, голосовые метрики, текстовый анализ эмоций и акустические признаки.
* Проверяет итоговый JSON и привязку доказательств к реальным `segment_id` расшифровки.
* Хранит промежуточные результаты map-reduce, чтобы повторная обработка длинного звонка не начиналась с нуля.
* Показывает звонки, которые требуют внимания руководителя.
* Ведёт разбор обращения: статус, ответственный, срок, эскалация, заметки и история действий.
...
2. ИИ Супервайзер импортирует событие и сохраняет ссылку на ревизию расшифровки.
3. Для подходящего звонка создаётся задание `call_summary`.
4. Обработчик согласовывает worker API v2, регистрируется и получает lease через `POST /job-leases`.
...
{% hint style="info" %}
Доказательства ИИ должны ссылаться на реальные `segment_id` из расшифровки. В интерфейсе технический идентификатор заменяется понятной подписью с номером фрагмента, участником и временем. Нажатие на подтверждение переводит к соответствующей реплике, поэтому вывод модели можно проверить по разговору.
{% endhint %}
...
* AI Supervisor Worker **1.7 build 34** на Mac с Apple Silicon и macOS 14 или новее.
* Сетевой доступ от Mac к веб-интерфейсу MikoPBX.
* Доступ в интернет для первой установки Ollama и загрузки локальных моделей.
* Достаточно оперативной памяти и свободного места для выбранного профиля.

{% hint style="warning" %}
Для обновления до worker API v2 сначала установите ModuleAISupervisor 1.73, затем AI Supervisor Worker 1.7 build 34. Старый обработчик не работает с API v2, а новый обработчик не работает с прежним API v1.
{% endhint %}
...
* темы, важные ситуации и другие выводы ИИ;
* оценки риска, качества, тон общения и сложность разговора, если эти данные доступны;
* подтверждающие фрагменты с понятными подписями и переходом к реплике в расшифровке;
...
{% hint style="warning" %}
Используйте ключ, созданный именно в ИИ Супервайзере. Ключ Local STT Worker предназначен только для модуля распознавания речи и здесь не подходит.
{% endhint %}
...
| **Анализ эмоций** | Текстовое обогащение эмоций по выбранным фрагментам. |
| **Акустический анализ** | Асинхронное добавление просодических и акустических признаков. |
...
### REST API и worker API v2

Модуль использует REST API v3 MikoPBX. Протокол взаимодействия с локальным обработчиком имеет отдельную версию worker API v2.
...
На каждом worker-запросе, кроме согласования контракта, требуется заголовок:

```text
X-MikoPBX-Worker-API-Version: 2
```

Загрузка записи дополнительно использует `X-MikoPBX-Worker-UID`, `X-MikoPBX-Job-Lease-Token` и `X-MikoPBX-Recording-Download-Token`. Эти значения нельзя помещать в URL или строку запроса.

Для JSON-запросов идентификатор `worker_uid` и, после получения задания, `lease_token` передаются в теле запроса. Итог `PUT /job-results/{id}` содержит закрытый объект `result` с полями `provider`, `model`, `summary`, `sentiment`, `risk_score`, `quality_score`, `metrics`, `indicators` и `raw_response`. Неизвестные поля и неверные типы отклоняются. Оценки должны находиться в диапазоне от 0 до 100, а размер `raw_response` не должен превышать 200000 байт.

{% hint style="warning" %}
Командные маршруты API v1 и их обработчики совместимости удалены. Старые custom-action URL отклоняются маршрутизатором MikoPBX как неизвестные или неподдерживаемые; модуль больше не формирует для них специальный ответ API v1. HTTP 426 `worker_upgrade_required` возвращается действующим ресурсом worker API v2, если заголовок `X-MikoPBX-Worker-API-Version` отсутствует, имеет неверный формат или содержит версию, отличную от `2`. Совместимого режима v1 нет.
{% endhint %}
