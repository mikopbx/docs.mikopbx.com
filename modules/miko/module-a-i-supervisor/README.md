...
{% hint style="info" %}
Доказательства ИИ должны ссылаться на реальные `segment_id` из расшифровки. Это позволяет перейти к подтверждающему фрагменту разговора и проверить вывод модели.
{% endhint %}
...
| **Разобрано ИИ** | Количество и доля звонков с сохранённым результатом анализа. |
...
Основные административные ресурсы:
...
{% hint style="warning" %}
Командные маршруты API v1 удалены. Worker-вызовы v1 возвращают HTTP 426 `worker_upgrade_required`, а удалённые административные действия — HTTP 410 `api_route_removed`. Совместимого режима v1 нет.
{% endhint %}
