...
**Local STT Worker** is an application that locally recognizes MikoPBX call recordings. It downloads one assigned job, prepares the audio with the bundled `ffmpeg` and `ffprobe`, runs WhisperKit/Core ML, and sends timestamped segments and technical diagnostics back to the PBX.
...
* Internet access for the first download of the required WhisperKit/Core ML model.

WhisperKit is an external Swift dependency of the application. `ffmpeg` and `ffprobe` are included in the worker build and do not need to be installed separately.
...
| **Models**      | Locally downloaded WhisperKit/Core ML models and storage usage.                          |
...
The summary shows the number of calls processed today, the percentage of successfully completed jobs, and the time of the latest call. Below it are the current activity, connection readiness, selected model, launch-at-login status, and three most recent jobs.
...
The model for new jobs is selected in MikoPBX on the **Model marketplace** tab. The worker automatically downloads the required model when it receives the first job that uses it.
...
| Location                   | Settings                                                                                                                                                                              |
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MikoPBX / module**       | Processing window, retention period, maximum recording duration, model, job language, recognition terms, processing profile, queue, keys, transcripts, and module log.          |
