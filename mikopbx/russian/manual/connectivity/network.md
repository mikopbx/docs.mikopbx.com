...
<figure><img src="../../.gitbook/assets/2 (25).png" alt=""><figcaption><p>Раздел "<strong>Сетевые интерфейсы</strong>"</p></figcaption></figure>
...
Если вы не указали адреса **DNS-серверов**, MikoPBX будет использовать резервные DNS-серверы по умолчанию. Для русской локализации сначала используется **77.88.8.8**, затем **1.1.1.1** и **8.8.8.8**; для остальных локализаций сначала используются **1.1.1.1** и **8.8.8.8**, затем **77.88.8.8**. DNS-серверы отвечают за преобразование доменных имен в IP-адреса, позволяя устройствам находить нужные ресурсы в сети.
...
* Если нам известен **внешний адрес** станции (IP или доменное имя) и **проброшены порты** АТС во внешний мир, то имеет смысл заполнить одно из полей: «**Внешний IP адрес Вашего маршрутизатора**» или «**Внешнее имя хоста вашего маршрутизатора**».
...
{% hint style="info" %}
При включенной NAT-настройке укажите внешний IP-адрес маршрутизатора или внешнее имя хоста. Если заполнены оба поля, доменное имя используется в приоритете, а внешний IP-адрес не используется. В режиме Dual-Stack внешнее имя хоста обязательно.
{% endhint %}

<figure><img src="../../.gitbook/assets/3 (10).png" alt=""><figcaption><p>Раздел "<strong>Топология сети</strong>"</p></figcaption></figure>
