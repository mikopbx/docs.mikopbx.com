...
{% hint style="warning" %}
In the default bridge mode the built-in MikoPBX firewall does not protect the web interface — the container cannot manage host iptables. See [External firewall for Docker](../../security/external-firewall-enforcement.md).
{% endhint %}
