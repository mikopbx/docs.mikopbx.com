...
This is where the [DID number](https://en.wikipedia.org/wiki/Direct_inward_dialing) comes to the rescue, which the provider can transmit during inbound calls. Knowing the DID number, you can define multiple inbound routes for the provider, each with a different DID, which solves the described problem.
