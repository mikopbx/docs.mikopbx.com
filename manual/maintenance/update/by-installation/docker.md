...
{% hint style="danger" %}
Если `/cf` и `/storage` не вынесены из контейнера в bind mounts или Docker volumes, удаление старого контейнера приведёт к потере настроек и данных.
{% endhint %}
...
Если используются именованные volumes или snapshot-хранилище, создайте их резервную копию средствами соответствующей системы хранения. При необходимости отдельно скопируйте `/storage`: этот каталог может занимать много места из-за записей разговоров.
