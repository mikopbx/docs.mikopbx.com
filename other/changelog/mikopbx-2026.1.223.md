...
\<figure>\<img src="../../.gitbook/assets/Release2026.1.GeoIP2Fail2Ban.png" alt="">\<figcaption>\<p>Флаги стран в списке заблокированных IP\</p>\</figcaption>\</figure> \<figure>\<img&#x20;
...
Управление доступно в **Настройки → Asterisk Manager → ARI пользователи**. Применения: разработка собственных IVR-приложений, интеграция с контакт-центрами, реализация кастомной логики обработки звонков.

<figure><img src="../../.gitbook/assets/AMI&#x26;ARIinGenSettings.png" alt=""><figcaption></figcaption></figure>
...
Для настройки откройте **Настройки → Провайдеры связи**, выберите тип **IAX Транк**, заполните параметры провайдера (Host, Username/Password, контекст) и настройте правила маршрутизации.

\<figure>\<img src="../../.gitbook/assets/Release2026.1.IAXTrunk.png" alt="">\<figcaption>\<p>Настройка IAX-транка\</p>\</figcaption>\</figure>
...
Пример настройки для Gmail: откройте **Система → Уведомления → Почта**, выберите провайдер **Gmail (OAuth2)**, нажмите **Авторизовать в Gmail**, войдите в Google-аккаунт и разрешите доступ — настройка завершена.

\<figure>\<img src="../../.gitbook/assets/Release2026.1.OAuth2Mail.png" alt="">\<figcaption>\<p>Настройка OAuth2 для Gmail\</p>\</figcaption>\</figure>
...
Реализовано полноэкранное консольное меню с системной информацией и метриками, отображаемое при подключении к консоли сервера (IPMI, виртуальная консоль, SSH).

Меню отображает ASCII-логотип MikoPBX, текущие IP-адреса всех интерфейсов, загрузку CPU, памяти и диска, количество активных звонков, время работы системы и информацию о последнем входе. Доступные действия: настройка сети, перезагрузка/выключение, просмотр логов, мониторинг системы, проверка целостности и управление модулями. Нажмите **0** для выхода в shell.

\<figure>\<img src="../../.gitbook/assets/Release2026.1.ConsoleMenu.png" alt="">\<figcaption>\<p>Консольное меню MikoPBX\</p>\</figcaption>\</figure>
...
Для импорта откройте **Настройки → Расширения**, нажмите **Импорт из CSV** и выберите файл с колонками: `number`, `mobile`, `email`, `name`, `username`, `secret`. Доступны три стратегии обновления: **Объединить** (обновить существующие, добавить новые), **Заменить** (перезаписать существующие) и **Пропустить** (добавить только новые).

Экспорт выполняется аналогично — нажмите **Экспорт в CSV** для скачивания файла со всеми внутренними номерами.

\<figure>\<img src="../../.gitbook/assets/Release2026.1.CSVImport.png" alt="">\<figcaption>\<p>Импорт сотрудников из CSV\</p>\</figcaption>\</figure>
...
Добавлено графическое отображение использования дискового пространства в разделе **Система → Хранилище → Использование диска**. Круговая диаграмма показывает распределение места по категориям: записи разговоров, системные логи, backup-файлы, модули и другие файлы — с процентами и размером в GB для каждой категории.

\<figure>\<img src="../../.gitbook/assets/Release2026.1.StorageVisualization.png" alt="">\<figcaption>\<p>Визуализация использования дискового пространства\</p>\</figcaption>\</figure>
...
Добавлено ограничение количества запросов с одного IP-адреса для защиты от DDoS и brute-force атак. Настраивается через параметр `PBXFirewallMaxReqSec` в разделе **Система → Безопасность → Firewall** (по умолчанию — 10 запросов в секунду).

#### Аудит событий аутентификации

Все события аутентификации теперь логируются: успешные и неудачные попытки входа, выход из системы, IP-адрес и время. Логи доступны в файле `/var/log/mikopbx/authentication.log` и через веб-интерфейс в разделе **Система → Логи → Аутентификация**.
...
| Метаданные записей | `/storage/usbdisk1/mikopbx/recordingstorage.db` **(новое)**      |
