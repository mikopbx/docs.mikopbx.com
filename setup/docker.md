...
{% hint style="warning" %}
Важное замечание, один из контейнеров использует проброс с SIP порта с изменением его значения 5060 -> 6060. В данном случае для корректной работы системы необходимо добавить внешнее значение SIP порта в настройках NAT в разделе сетевых интерфейсов MikoPBX. Это можно сделать задав соответвующее значение переменной окружения EXTERNAL\_SIP\_PORT=6060
{% endhint %}
